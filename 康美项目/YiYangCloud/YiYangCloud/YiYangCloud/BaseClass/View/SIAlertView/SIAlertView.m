    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            ...
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                ...
                button.frame = CGRectMake(BUTTON_PADDING_LEFT, y, self.containerView.bounds.size.width - BUTTON_PADDING_LEFT * 2, BUTTON_HEIGHT);
...
- (void)updateTitleLabel
{
	if (self.title) {
		if (!self.titleLabel) {
            ...
            _colorView.backgroundColor = UIColorFromRGB(0xf1f1f1);
            [self.containerView addSubview:_colorView];
            
//            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, _colorView.bounds.size.height - 0.6, _colorView.bounds.size.width, 0.6)];
//            lineView.backgroundColor = [Resource lightBlackColor];
//            [_colorView addSubview:lineView];
...
//	UIImage *highlightedImage = nil;
	switch (item.type) {
		case SIAlertViewButtonTypeCancel:
            ...
            [button setBackgroundColor:UIColorFromRGB(0x3b96ff)];
			break;
