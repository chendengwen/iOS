...
-(void)beginLoginOperation:(NSString *)phoneNum password:(NSString *)psd{
    ...
    [params setObject:phoneNum forKey:kUserName];
    [params setObject:psd forKey:kPassword];
    
    [[AFNetworkManager sharedAFNetworkManager] PostRequestUrl:[URLs getFullLoginsAPI:fuc_loginAuth] parameters:params successBlock:^(JsonResult *result) {
        [self successed:result phoneNum:phoneNum password:psd];
...
-(void)successed:(JsonResult *)result phoneNum:(NSString *)phoneNum password:(NSString *)psd{
    //判断是否第一次使用
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"AppStateFirstLoad"]) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"AppStateFirstLoad"];
        [[AppCacheManager sharedAppCacheManager] setAppCache:@(AppStateFirstLoad) forKey:kAppStatus];
    }else{
        // 保存用户数据  保存缓存
