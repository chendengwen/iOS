...
static NSString * const KMRouteParamPattern=@":[a-zA-Z0-9-_][^/]+";
static NSString * const KMRouteParamNamePattern=@":[a-zA-Z0-9-_]+";
static NSString * const WLPRouteParamMatchPattern=@"([^/]+)";
@interface KMRegularExpression ()
@property(nonatomic,strong)NSArray * routerParamNamesArr;
@end

@implementation KMRegularExpression

+(KMRegularExpression *)expressionWithPattern:(NSString *)pattern{
    NSError *error;
    KMRegularExpression * exp =[[KMRegularExpression alloc]initWithPattern:pattern options:NSRegularExpressionCaseInsensitive error:&error];
    if (error) {
        NSLog(@"%@",error);
    }
    return exp;
...
-(instancetype)initWithPattern:(NSString *)pattern options:(NSRegularExpressionOptions)options error:(NSError * _Nullable __autoreleasing *)error{
    ...
    NSString *transformedPattern = [KMRegularExpression transfromFromPattern:pattern];
...
/**
 通过一个url生产出一个KMMatchResult，检查是否匹配，并且将url路径中的关键字参数对应的值放入KMMatchResult对象中
 
 @param string 一个需要匹配的url
 @return 匹配结果KMMatchResult对象
 */
-(KMMatchResult *)matchResultForString:(NSString *)string{
    ...
    KMMatchResult * result = [[KMMatchResult alloc]init];
    if (array.count == 0) {
        return result;
    }
    result.matched = YES;
...
+(NSString*)transfromFromPattern:(NSString *)pattern{
    ...
    NSError * err;
    NSRegularExpression * paramNamePatternEx = [NSRegularExpression regularExpressionWithPattern:KMRouteParamNamePattern options:NSRegularExpressionCaseInsensitive error:&err];
    /*
...
+(NSArray<NSString *> * )paramPatternStringsFromPattern:(NSString *)pattern{
    NSError *err;
    NSRegularExpression * paramPatternEx = [NSRegularExpression regularExpressionWithPattern:KMRouteParamPattern options:NSRegularExpressionCaseInsensitive error:&err];
...
+(NSArray *)routeParamNamesFromPattern:(NSString *)pattern{
    NSRegularExpression *paramNameEx = [NSRegularExpression regularExpressionWithPattern:KMRouteParamNamePattern options:NSRegularExpressionCaseInsensitive error:nil];
