...
- (NSUInteger) platformType
{
    ...
    if ([platform hasPrefix:@"iPad5,1"])            return UIDevice5GiPad;
...
- (NSString *) typeCpu
{
    switch ([self platformType])
    {
        ...
        case UIDevice5ciPhone: return IPHONE_5_CPUTYPE;
        case UIDevice5siPhone: return IPHONE_5s_CPUTYPE;
        case UIDevice6iPhone: return IPHONE_6_CPUTYPE;
        case UIDevice6PiPhone: return IPHONE_6P_CPUTYPE;
        ...
        case UIDevice1GiPad: return @"N45";
        case UIDevice2GiPad: return @"N72";
        case UIDevice3GiPad: return IPOD_4G_CPUTYPE;
        case UIDevice4GiPad: return IPOD_5G_CPUTYPE;
...
+ (NSString *)wifiMacAddressString
{
    ...
    if ((mgmtInfoBase[5] = if_nametoindex("en0")) == 0)
        errorFlagString = @"if_nametoindex failure";
    else
    {
        // Get the size of the data available (store in len)
        if (sysctl(mgmtInfoBase, 6, NULL, &length, NULL, 0) < 0)
            errorFlagString = @"sysctl mgmtInfoBase failure";
        else
        {
            // Alloc memory based on above call
            if ((msgBuffer = malloc(length)) == NULL)
                errorFlagString = @"buffer allocation failure";
            else
            {
                // Get system information, store in buffer
                if (sysctl(mgmtInfoBase, 6, msgBuffer, &length, NULL, 0) < 0)
                    errorFlagString = @"sysctl msgBuffer failure";
            }
        }
    }
    
    // Befor going any further...
    if (errorFlagString != NULL)
    {
        NSLog(@"Error getting MAC address: %@", errorFlagString);
        free(msgBuffer);
        return nil;
    }
