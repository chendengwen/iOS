...
-(int)initSocket
{
    ...
    /**
     ...
     其中h代表主机（host）字节序，n代表网络（network）字节序，
     l代表long，htons 是 h、to、n、l 的组合
     */
    ...
    // 3. listen 监听套接字
    // listenfd 为需要进入监听状态的套接字，BACKLOG 为请求队列的最大长度
    ...
    // m_clientaddr 保存了客户端的IP地址和端口号，而 listenfd 是服务器端的套接字
