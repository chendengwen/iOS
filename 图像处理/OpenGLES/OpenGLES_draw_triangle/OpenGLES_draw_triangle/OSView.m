...
// MARK: - 步骤0  重写下面的方法，将view的layer层变为 CAEAGLayer 类型 简单的很
+(Class)layerClass{
...
-(void)configure{
    ...
    eagLayer.drawableProperties = @{kEAGLDrawablePropertyRetainedBacking:@(false),
                                    kEAGLColorFormatRGBA8:@(true)};
...
-(void)createTriangleVertices{
    ...
    // 设置读取Position数据时从第1字节开始读取，每8字节读一次  顶点数量为3
    glVertexAttribPointer(GLKVertexAttribPosition, // 指示绑定的缓存包含的是顶点位置的信息
                          ...
                          NULL); // 告诉opengl 可以从绑定数据的开始位置访问数据 BUFFER_OFFSET(0)
